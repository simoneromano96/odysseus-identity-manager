...
const main = async () => {
  ...
  passport.use(
    ...
    new Strategy(
      {
        client: openIdClient,
        params: { scope: "openid offline email profile phone address" },
      },
      (
        tokenSet: TokenSet,
        userinfo: UserinfoResponse,
        done: (err: string | null, user?: any) => void
      ) => {
  ...
  app.get("/auth/login/callback", (req, res, next) => {
    ...
    passport.authenticate("oidc", { successRedirect: "/auth/am-i-auth" })(
      req,
      res,
      next
    );
  ...
  app.get("/auth/logout", (req, res, next) => {
    ...
    logoutUrl.pathname = "/oauth2/sessions/logout";
